					...
					<div class="row">
						...
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="padding-left:2px;"><input class="span10 date-picker" name="lastStart" id="lastStart"   type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="开始日期" title="开始日期"/></td>
								<td style="padding-left:2px;"><input class="span10 date-picker" name="lastEnd" id="lastEnd"  type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="结束日期" title="结束日期"/></td>
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 && var.STATUS == 0}">
													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.ORDER_ID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="发货"></i>
													</a>
													</c:if>
											</div>
											<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<c:if test="${QX.edit == 1 && var.STATUS == 0}">
															<li>
																<a style="cursor:pointer;" onclick="edit('${var.ORDER_ID}');" class="tooltip-success" data-rel="tooltip" title="aaa发货">
																	<span class="green">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<c:if test="${QX.edit == 1 }">
									<a class="btn btn-mini btn-success" onclick="makeAll('确定要确认发货?');" title="批量发货" >批量发货</a>
									</c:if>
								</td>
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要确认发货?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>order/updateAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
								//beforeSend: validateData,
								cache: false,
								success: function(data){
									if(data.list[0].msg=="nok"){
										alert("仅将未发货订单发货");
									}else if(data.list[0].msg=="null"){
										alert("请选择未发货订单发货");
									}
