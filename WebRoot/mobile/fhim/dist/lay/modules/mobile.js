...
function(i) {
    ...
    f = function(i, e, a) {
        return ['<div class="layim-panel' + (e ? " layui-m-anim-left": "") + '">', '<div class="layim-title" style="background-color: {{d.base.chatTitleColor}};">', "<p>", a ? '<i class="layui-icon layim-chat-back" layim-event="back">&#xe603;</i>': "", '{{ d.title || d.base.title }}<span class="layim-chat-status"></span>', "{{# if(d.data){ }}", '{{# if(d.data.type === "group"){ }}', '<i class="layui-icon layim-chat-detail" layim-event="detail">聊天记录</i>', "{{# } }}", '{{# if(d.data.type === "friend"){ }}', '<i class="layui-icon layim-chat-detail" layim-event="mchatlog">聊天记录</i>', "{{# } }}","{{# } }}", "</p>", "</div>", '<div class="layui-unselect layim-content">', i, "</div>", "</div>"].join("")
    },
