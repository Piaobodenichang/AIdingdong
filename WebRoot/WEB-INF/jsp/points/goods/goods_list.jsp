								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.GOODS_DESCRIBE}</td>
											<td class='center'>${var.GOODS_STATE}</td>
											<td class='center'>												
												<img style="margin-top: -3px;" src="img/${var.GOODS_PICTURE}" height="35px" width="35px">												
											</td>
											<td class="center">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									...
									<c:if test="${QX.edit == 1 }">
									<a class="btn btn-mini btn-success" onclick="editAlls('确定要上架选中的商品吗?');" title="批量上架" >上架</a>
									</c:if>
									<c:if test="${QX.edit == 1 }">
									<a class="btn btn-mini btn-success" onclick="editAllx('确定要下架选中的商品吗?');" title="批量下架" >下架</a>
									</c:if>
									<c:if test="${QX.del == 1 }">
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要上架选中的商品吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>goods/editAlls.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要下架选中的商品吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>goods/editAllx.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
								//beforeSend: validateData,
								cache: false,
